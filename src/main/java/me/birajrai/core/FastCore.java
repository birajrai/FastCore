package me.birajrai.core;

import me.birajrai.core.commands.*;
import me.birajrai.core.commands.bungee.*;
import me.birajrai.core.commands.economy.BalanceCommand;
import me.birajrai.core.commands.economy.BountyCommand;
import me.birajrai.core.commands.economy.PayCommand;
import me.birajrai.core.commands.economy.staff.EconomyCommand;
import me.birajrai.core.commands.economy.staff.UnbountyCommand;
import me.birajrai.core.commands.punishments.*;
import me.birajrai.core.commands.staff.*;
import me.birajrai.core.commands.stats.StatsCommand;
import me.birajrai.core.commands.stats.staff.ResetStatsCommand;
import me.birajrai.core.data.*;
import me.birajrai.core.grants.Grant;
import me.birajrai.core.grants.GrantType;
import me.birajrai.core.listeners.*;
...
import me.birajrai.core.runnables.*;
...
public final class FastCore extends JavaPlugin {

    private static FastCore instance;
    ...
    public Map<UUID, me.birajrai.core.tags.Tag> tag = new HashMap<>();
    ...
    public Map<UUID, Map<me.birajrai.core.server.Server, Location>> last_location = new HashMap<>();
    private PermissionManagement permissionManagement;

    public static FastCore getInstance() {
        return instance;
    }
...
//        Bukkit.getConsoleSender().sendMessage("FastCore is loading...");
            ...
            public void run() {
                ...
                for (Player players : Bukkit.getOnlinePlayers()) {
                    if (player_permissions.containsKey(players.getUniqueId()))
                        Permissions.setup(yoPlayer.getYoPlayer(players));
                ...
                Bukkit.getConsoleSender().sendMessage("FastCore has successfully loaded.");
            }
    ...
    public void onDisable() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (last_location.get(player.getUniqueId()) == null) {
                Map<me.birajrai.core.server.Server, Location> location = new HashMap<>();
                location.put(me.birajrai.core.server.Server.getServer(player), player.getLocation());
                last_location.put(player.getUniqueId(), location);
            }

            last_location.get(player.getUniqueId()).put(me.birajrai.core.server.Server.getServer(player), player.getLocation());
        }

        for (Map.Entry<UUID, Map<me.birajrai.core.server.Server, Location>> entry : last_location.entrySet()) {
            for (Map.Entry<me.birajrai.core.server.Server, Location> data : entry.getValue().entrySet()) {
    ...
    public void registerData() {
        ...
        if (!worldData.config.contains("Servers")) {
            ...
            me.birajrai.core.server.Server server = new me.birajrai.core.server.Server("server", worlds, spawn);
            server.create();
        }
    ...
    private void registerServers() {
        for (String server : worldData.config.getConfigurationSection("Servers").getKeys(false)) {
            ...
            me.birajrai.core.server.Server newServer = new me.birajrai.core.server.Server(worldData.config.getString("Servers." + server + ".Name"), worldData.config.getStringList("Servers." + server + ".Worlds"), spawn);

            me.birajrai.core.server.Server.getServers().put(newServer.getName(), newServer);
        }
    }

    private void registerRanks() {
        for (Permission permission : Permissions.getAllServerPermissions()) {
            if (permission.getName().contains("fcore.grant."))
        ...
        for (String rank : getConfig().getConfigurationSection("Ranks").getKeys(false)) {
            Permission permission = new Permission("fcore.grant." + getConfig().getString("Ranks." + rank + ".ID").toLowerCase());
    ...
    private void registerTags() {
        for (Permission permission : getServer().getPluginManager().getPermissions()) {
            if (permission.getName().contains("fcore.tags."))
                getServer().getPluginManager().removePermission(permission);
        }

        for (String tag : getConfig().getConfigurationSection("Tags").getKeys(false)) {
            Permission permission = new Permission("fcore.tags." + getConfig().getString("Tags." + tag + ".ID").toLowerCase());
            permission.setDescription("Permission");
            ...
            me.birajrai.core.tags.Tag.getTags().put(ID, new Tag(ID, prefix, display));
        }
    }
        ...
        for (String player : playerData.config.getKeys(false)) {
            if (playerData.config.contains(player + ".LastLocation")) {
                ...
                for (String locations : playerData.config.getConfigurationSection(player + ".LastLocation").getKeys(false)) {
                    ...
                    me.birajrai.core.server.Server server = Server.getServer(playerData.config.getString(player + ".LastLocation." + locations + ".Server"));
    ...
    private void registerCommands() {
        ...
        getCommand("Fastreload").setExecutor(new ReloadCommand());
    }
}
