...
public class AltsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("fcore.alts")) {
        ...
        for (String alt : plugin.playerData.config.getKeys(false)) {
            ...
            if (yoPlayer.getIP().equalsIgnoreCase(yoTarget.getIP())
                ...
                if (!target.getName().equalsIgnoreCase(yoPlayer.getPlayer().getName())) {
                    ...
                    if (plugin.muted_players.containsKey(yoPlayer.getPlayer().getUniqueId()))
                        display = "&6" + yoPlayer.getPlayer().getName();
                    if (plugin.banned_players.containsKey(yoPlayer.getPlayer().getUniqueId()))
                        display = "&c" + yoPlayer.getPlayer().getName();
                    if (plugin.blacklisted_players.containsKey(yoPlayer.getPlayer().getUniqueId()))
                        display = "&4" + yoPlayer.getPlayer().getName();
