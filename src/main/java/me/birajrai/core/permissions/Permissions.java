package me.birajrai.core.permissions;
...
public class Permissions {
    ...
    private static final FastCore splugin = FastCore.getInstance();
    private final FastCore plugin = FastCore.getInstance();
    private yoPlayer player;
    private Rank rank;

    public Permissions(yoPlayer player) {
        this.player = player;
    }
                ...
                for (Permission plugin_permission : getAllServerPermissions()) {
                    if (plugin_permission.getName().contains(permission.split("\\.")[0])) {
                        if (permission.startsWith("-"))
                            attachment.setPermission(plugin_permission.getName().replaceFirst("-", ""), false);
    ...
    public static void refresh(yoPlayer player) {
        ...
        try {
            nativePlayer.removeAttachment(splugin.player_permissions.get(nativePlayer.getUniqueId()));
